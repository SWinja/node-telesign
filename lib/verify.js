...
(function() {
  ...
  Verify.smart = function(options, callback) {
    if (!options || typeof options !== 'object') {
      throw new Error('Error calling Verify Smart - no params object provided.');
    } else if (!callback || typeof callback !== 'function') {
      throw new Error('Error calling Verify Smart - no callback function provided.');
    } else if (!options.phoneNumber) {
      return callback('Error calling Verify Smart - "phoneNumber" not provided in the request params.');
    } else if (!options.ucid) {
      return callback('Error calling Verify Smart - "ucid" not provided in the request params.');
    }
    ...
    var fields = {
      ...
      language: options.language,
      //verify_code: options.verifyCode,
      push_message: options.pushMessage,
      sms_message: options.template,
      tts_message: options.voiceMessage,
      preference: options.preferMethod,
      ignore_risk: options.ignoreRisk
    };
    console.log("::: npm module TeleSign:Verify.smart().fields: ", fields);

    sendRequest.request({
      method: 'POST',
      resource: '/verify/smart',
      qs: null,
      fields: fields
    }, function(err, data) {
      return callback(err, data);
