...
(function() {
  ...
  SendRequest.request = function(options, fn) {
    var headers = authorize.getHeaders(options.method, options.resource, options.fields, options.fieldsJson);
    ...
    if (options.fieldsJson) {
      requestOptions.body = options.fieldsJson;
    }

    // DEBUG
    // console.warn("::: npm module SendRequest.request: ", requestOptions);
